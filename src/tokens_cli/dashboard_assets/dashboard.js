...
const I18N_PATTERNS = Object.freeze([
  ...
  [/^\/ (\d+) · 图鉴阶位并非全球稀有度$/,(_,total)=>`/ ${total} · Catalog tiers do not represent global rarity`],
  [/^本地数据宇宙居民 · (.+)$/,(_,dom)=>`Resident of the local data universe · ${dom}`],
  ...
  [/^([\d.]+)(万|亿)$/,(_,amount,unit)=>{const value=Number(amount)*(unit==='亿'?1e8:1e4),label=value>=1e9?(value/1e9).toFixed(2)+'B':value>=1e6?(value/1e6).toFixed(2)+'M':value>=1e3?(value/1e3).toFixed(1)+'K':String(Math.round(value));return label;}],
  ...
  [/^达成于 (\d{4}-\d{2}-\d{2})$/,(_,d)=>'Unlocked on '+d],
  [/^(\d{1,3})% · 还差 (.+)$/,(_,p,v)=>p+'% · '+v+' to go'],
...
const I18N_KEYED_ZH = Object.freeze({
  'Exact details':'精确明细',
  ...
  'Retry':'重试',
  'Loads when scrolled into view':'进入视野后加载',
  'Caliber':'口径',
...
function localizeKeyedElements(root=document){
  if(!(root instanceof Document||root instanceof Element))return;
  const elements=[];
  if(root instanceof Element&&root.hasAttribute('data-i18n-key'))elements.push(root);
  elements.push(...root.querySelectorAll('[data-i18n-key]'));
  elements.forEach(element=>{const key=element.dataset.i18nKey;element.textContent=dashboardLanguage==='zh'?(I18N_KEYED_ZH[key]||key):key;});
}
function localizeTree(root=document.body){
  if(!root)return;
  if(root.nodeType===Node.TEXT_NODE){localizeTextNode(root);return;}
  localizeKeyedElements(root);
...
function human(n){ if(dashboardLanguage!=='zh') return metric(n); if(n>=1e8) return (n/1e8).toFixed(1)+'亿'; if(n>=1e4) return (n/1e4).toFixed(1)+'万'; return String(Math.round(n)); }
...
/* 集中聚合 helper：求和 / 模型构成 / 峰值时段，全文件统一走这里 */
const sumList=values=>values.reduce((a,b)=>a+(b||0),0);
const sumBy=(rows,key)=>rows.reduce((a,row)=>a+(row[key]||0),0);
const sumWhere=(items,fn)=>items.reduce((a,item)=>a+(fn(item)||0),0);
const modelSum=(models,allowed)=>sumWhere(Object.entries(models||{}),([m,v])=>allowed.has(m)?v:0);
const modelTotals=rows=>{const mt={};rows.forEach(r=>Object.entries(r.models||{}).forEach(([m,v])=>mt[m]=(mt[m]||0)+v));return mt;};
const sortedModels=(models,tieByName=false)=>Object.entries(models||{}).sort((a,b)=>b[1]-a[1]||(tieByName?a[0].localeCompare(b[0]):0));
const topModelOf=(models,tieByName=false)=>sortedModels(models,tieByName)[0]||null;
const peakHourOf=hours=>hours&&hours.length?hours.indexOf(Math.max(...hours)):-1;
...
const CALIBER_NOTES=Object.freeze({
  ...
  cache:'Cache hit share = cache-read tokens over total. Not a currency-saving estimate.',
  ledger:'Coverage denominator = all-model total at the current granularity; time-probe focus does not narrow it.',
});
function caliberNote(id){const copy=CALIBER_NOTES[id];return copy?'<details class="caliber"><summary data-i18n-key="Caliber">Caliber</summary><p data-i18n-key="'+esc(copy)+'">'+esc(copy)+'</p></details>':'';}
function renderCaliberNotes(){[['caliber-cache','cache'],['caliber-ledger','ledger'],['caliber-block','block']].forEach(([target,id])=>{const el=document.getElementById(target);if(el)el.innerHTML=caliberNote(id);});localizeKeyedElements(document);}
...
function renderKPI(){
  ...
  const total=sumBy(rows,'total'); lastTotal=total;
  const calls=sumBy(rows,'calls');
  const mtot=modelTotals(rows);
  const domEntry=topModelOf(mtot), dom=domEntry?domEntry[0]:'—';
...
function trendPreviewCopy(row){
  if(!row)return '';
  const dom=topModelOf(row.models);
...
function renderScrubPreview(announce=false){
  ...
  stacks.forEach((el,index)=>{const preview=index===scrubState.index;el.classList.toggle('scrub-preview',preview);el.classList.toggle('scrub-away',!!row&&!preview);if(preview)el.setAttribute('aria-current','true');else el.removeAttribute('aria-current');});
...
function renderBar(){
  ...
  rows.forEach((r,i)=>{
    ...
    state.models.forEach(m=>{ const v=r.models[m]||0; if(v<=0) return;
      ...
      segs+='<rect class="seg model-mark" data-model="'+esc(m)+'"'+dataSignalAttrs('model',m,pretty(m),v,'trend',false)+' x="'+x.toFixed(1)+'" y="'+y.toFixed(1)+'" width="'+bw.toFixed(1)+'" height="'+h.toFixed(1)+'" rx="2" fill="'+modelColor(m)+'"><title>'+esc(r.period)+' · '+esc(pretty(m))+': '+fmt(v)+' ('+pct(v,r.total)+')</title></rect>';
      y0-=h;
    });
...
function renderDonut(){
  ...
  entries.forEach(([m,v])=>{
    ...
    if(frac>=0.999){
      p.push('<circle class="slice model-mark" data-model="'+esc(m)+'"'+dataSignalAttrs('model',m,pretty(m),v,'composition')+' cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="'+modelColor(m)+'"><title>'+esc(pretty(m))+' '+pct(v,total)+'</title></circle>');
    }else{
      const large=frac>0.5?1:0;
      const x0=cx+r*Math.cos(a0), y0=cy+r*Math.sin(a0), x1=cx+r*Math.cos(a1), y1=cy+r*Math.sin(a1);
      p.push('<path class="slice model-mark" data-model="'+esc(m)+'"'+dataSignalAttrs('model',m,pretty(m),v,'composition')+' d="M '+cx+' '+cy+' L '+x0.toFixed(2)+' '+y0.toFixed(2)+' A '+r+' '+r+' 0 '+large+' 1 '+x1.toFixed(2)+' '+y1.toFixed(2)+' Z" fill="'+modelColor(m)+'"><title>'+esc(pretty(m))+' '+pct(v,total)+'</title></path>');
    }
    angle=a1;
  });
...
function renderTrendLegend(){
  const box=document.getElementById('trend-legend');if(!box)return;
  const rows=DATA[state.gran]||[],all=sumBy(rows,'total');
  box.innerHTML=DATA.models.map(m=>{const on=state.models.has(m),value=sumWhere(rows,r=>(r.models||{})[m]||0);return '<button type="button" class="tl-item'+(on?'':' off')+'" aria-pressed="'+on+'" data-model-toggle="'+esc(m)+'"'+dataSignalAttrs('model',m,pretty(m),value,'legend',false)+'><span class="tldot" style="background:'+modelColor(m)+'"></span><span>'+esc(pretty(m))+'</span><em>'+pct(value,all)+'</em></button>';}).join('');
...
function renderTable(){
  ...
  document.querySelectorAll('#tbody tr[data-period]').forEach(tr=>{
    tr.addEventListener('pointerenter',()=>{const index=allRows.findIndex(r=>r.period===tr.dataset.period);if(index>=0)setScrubPreview(index,'pointer',false,false);});
    tr.addEventListener('pointerleave',()=>clearScrub());
  });
}
...
function renderFilters(){
  ...
  box.innerHTML=DATA.models.map(m=>{
    ...
    return '<label class="chip'+(on?'':' off')+'"'+dataSignalAttrs('model',m,pretty(m),0,'filters',false)+'><input type=checkbox value="'+esc(m)+'" '+(on?'checked':'')+'><span class=cdot style="background:'+modelColor(m)+'"></span>'+esc(pretty(m))+'</label>';
  }).join('');
...
function lbRows(items,sessionRows=false,kind='project'){
  ...
  const filtered=items.map(it=>{const parts=sortedModels(it[3]).filter(([m])=>state.models.has(m)),total=parts.reduce((a,[,v])=>a+v,0);return {it,parts,total};}).filter(x=>x.total>0);
  ...
  return filtered.map(({it,parts,total},i)=>{
    const w=total/max*100,full=it[2]||it[0],dom=parts[0],comp=parts.map(([m,v])=>'<i style="width:'+(v/total*100).toFixed(1)+'%;background:'+modelColor(m)+'" title="'+esc(pretty(m))+' '+pct(v,total)+'"></i>').join('');
...
function shareStats(){
  const days=DATA.day||[],total=sumBy(days,'total'),calls=sumBy(days,'calls'),h=DATA.hourly||[],peak=peakHourOf(h);const dom=topModelOf(modelTotals(days));
...
function renderProbe(){
  ...
  const row=selectedRows()[0], dom=row?topModelOf(row.models):null;
...
function trailModelHTML(scope){
  ...
  const options=scope.models.map((item,index)=>{const previous=trailModelPrevious(item.model,scope),delta=previous?item.total-previous.total:null,on=trailState.model===item.model;return '<button type=button class="trail-option '+(on?'on':'')+'" data-trail-model="'+esc(item.model)+'" data-trail-roving="'+(index===0?'0':'-1')+'" tabindex="'+(index===0?'0':'-1')+'" style="--trail-color:'+modelColor(item.model)+'"><i class=trail-dot></i><b>'+esc(pretty(item.model))+'</b><strong>'+human(item.total)+' tk</strong><small>'+fmt(item.calls)+' 条调用记录 · '+esc(trailDeltaText(delta,previous))+'</small></button>';}).join('');
...
function resolveSignalEvidence(signal){
  ...
  if(signal.type==='model'){
    const rows=selectedRows(),sourceByPeriod=Object.fromEntries((DATA[state.gran]||[]).map(row=>[row.period,row])),total=sumWhere(rows,row=>row.models[signal.id]||0),calls=sumWhere(rows,row=>((sourceByPeriod[row.period]?.model_calls||{})[signal.id]||0)),all=sumBy(rows,'total');
  ...
  if(signal.type==='project'){
    const item=scopedEntities('project').find(row=>row[2]===signal.id),models=item?.[3]||{},dominant=topModelOf(models),total=item?.[1]||0,all=sumBy(selectedRows(),'total');
...
function signalScopeRoot(source){if(!source)return null;const scope=source.dataset?.signalScope;if(!scope||scope==='filters')return null;if(scope==='flow')return document.getElementById('flow-map');if(scope==='top')return source.closest('#section-top');if(scope==='trend')return source.closest('#section-trend');if(scope==='composition')return source.closest('.card');if(scope==='project')return source.closest('#section-project');return source.closest('.card,section');}
...
function modelColor(model){const palettes=DATA.colors||{},active=palettes[effectiveTheme()]||palettes.light||palettes.dark||{};return active[model]||'var(--accent)';}
function activeModelColors(){return (DATA.models||[]).map(modelColor);}
function themeToken(name,fallback){const value=getComputedStyle(document.documentElement).getPropertyValue(name).trim();return value||fallback;}
...
const lazyState={};
function ensureLazyPlaceholder(card){
  let placeholder=card.querySelector(':scope > .lazy-placeholder');
  if(!placeholder){
    placeholder=document.createElement('div');placeholder.className='lazy-placeholder';placeholder.setAttribute('data-i18n-key','Loads when scrolled into view');placeholder.textContent='进入视野后加载';card.appendChild(placeholder);localizeKeyedElements(placeholder);
  }
  return placeholder;
}
...
function renderLazy(name,force=false){
  ...
  if(!force&&!lazyState[name]?.visible){ensureLazyPlaceholder(card);card.classList.add('lazy-pending');card.setAttribute('aria-busy','true');lazyState[name]=Object.assign({},lazyState[name],{dirty:true,rendered:false,status:'pending'});return false;}
...
function initLazyRendering(){
  document.querySelectorAll('[data-lazy]').forEach(card=>{const name=card.dataset.lazy;lazyState[name]={visible:false,dirty:true,rendered:false};ensureLazyPlaceholder(card);card.classList.add('lazy-pending');});
...
const refreshAutoTheme=()=>{if(currentTheme()==='auto')refreshThemeVisuals();};
...
document.getElementById('logo').addEventListener('click',()=>{
  ...
  const h=DATA.hourly||[],pi=peakHourOf(h);
  const top=(DATA.top_cwds&&DATA.top_cwds[0])?DATA.top_cwds[0][0]:'—';
  const calls=sumBy(DATA.day||[],'calls');
  const F=[
...
/* ---- 模块开关（附加功能可勾选 + 记忆）---- */
const MODS_KEY='tk-mods', MOD_DEFAULT={block:true,rhythm:true,modes:true,provenance:true,project:true,reuse:true,flow:true,almanac:true,badges:true,top:true};
...
function renderRhythm(){
  ...
  const dayTotals=matrix.map(sumList);
  ...
  const hs=Array(24).fill(0);matrix.forEach(a=>a.forEach((v,i)=>hs[i]+=v));const total=sumList(hs),sum=(a,b)=>sumList(hs.slice(a,b));
...
function workModeDailyRows(data,selectedModels,focus=null){
  ...
  return [...(data.day||[])].filter(row=>!focus||focus.has(row.period)).sort((a,b)=>a.period.localeCompare(b.period)).map(row=>{
    ...
    const activeHours=hourly.filter(value=>value>0).length,circularHours=hourly.concat(hourly.slice(0,3)),fourHourPeak=hourly.reduce((best,_,hour)=>Math.max(best,sumList(circularHours.slice(hour,hour+4))),0);
    let projectCount=0;(detail.cwds||detail.top_cwds||[]).forEach(project=>{const projectTotal=modelSum(project[3],models);if(projectTotal>0)projectCount++;});
...
function renderProjectLens(){
  ...
  const chosen=catalog.find(x=>x.id===selectedProject),rows=projectRows(selectedProject),total=sumBy(rows,'total'),overall=sumBy(selectedRows(),'total'),peak=rows.reduce((a,r)=>!a||r.total>a.total?r:a,null);const dom=topModelOf(modelTotals(rows));
...
function renderReuseRiver(){
  const rows=selectedReuseRows(),svg=document.getElementById('reuse-chart'),panel=document.getElementById('reuse-panel'),selectedTotal=sumWhere(rows,r=>sumList(r.slice(1)));
  ...
  const totalAll=sumWhere(original,r=>sumList(series.map(i=>r[i]))),cacheAll=sumWhere(original,r=>r[3]||0);document.getElementById('reuse-summary').textContent='缓存复用 '+pct(cacheAll,totalAll);
...
function renderFlow(){
  const svg=document.getElementById('flow-map'),raw=currentFlow(),selectedTotal=sumBy(selectedRows(),'total');
  const pm=(raw.project_model||[]).filter(x=>state.models.has(x[2])&&x[3]>0),ms=(raw.model_session||[]).filter(x=>state.models.has(x[0])&&x[3]>0);
  const groupSum=(arr,key,val)=>{const o={};arr.forEach(x=>o[x[key]]=(o[x[key]]||0)+(x[val]||0));return o;};
  const pmModels=groupSum(pm,2,3),msModels=groupSum(ms,0,3),allModels=new Set([...Object.keys(pmModels),...Object.keys(msModels)]),mt={};allModels.forEach(m=>mt[m]=Math.max(pmModels[m]||0,msModels[m]||0));
  const modelIds=Object.keys(mt).filter(m=>state.models.has(m)).sort((a,b)=>(mt[b]||0)-(mt[a]||0)).slice(0,7),modelSet=new Set(modelIds),modelPM=pm.filter(x=>modelSet.has(x[2])),modelMS=ms.filter(x=>modelSet.has(x[0])),pt=groupSum(modelPM,1,3),st=groupSum(modelMS,2,3);
  ...
  const projectColor=themeToken('--flow-project','#4a6fa5'),sessionColor=themeToken('--flow-session','#8f5fa8');
  let p=['<defs>'];modelIds.forEach((m,i)=>{const c=modelColor(m);p.push('<linearGradient id="flow-g-'+i+'" x1="0" x2="1"><stop stop-color="'+c+'" stop-opacity=".25"/><stop offset=".5" stop-color="'+c+'"/><stop offset="1" stop-color="'+c+'" stop-opacity=".35"/></linearGradient>');});p.push('</defs><text class="flow-col" x="55" y="28">PROJECT</text><text class="flow-col" x="520" y="28">MODEL</text><text class="flow-col" x="942" y="28">SESSION</text>');
...
function dataCommands(){
  const days=DATA.day||[],top=[...days].sort((a,b)=>b.total-a.total)[0],h=DATA.hourly||[],peak=peakHourOf(h),mt=modelTotals(days);
  const out=[];if(top)out.push({ic:'🔍',t:'最高 Token 日 · '+top.period+' · '+human(top.total),k:'数据',run:()=>{setGran('day');setTimeout(()=>toggleFocus(top.period),30);}});if(peak>=0)out.push({ic:'🌙',t:'最活跃时刻 · '+String(peak).padStart(2,'0')+':00 · '+human(h[peak]),k:'数据',run:()=>document.getElementById('section-rhythm').scrollIntoView({behavior:scrollBehavior()})});sortedModels(mt).forEach(([m,v])=>out.push({ic:'🤖',t:pretty(m)+' · '+human(v)+' · '+(v/sumList(Object.values(mt))*100).toFixed(1)+'%',k:'模型',run:()=>setModels([m],'Solo · '+pretty(m))}));return out;
}
function secretCommand(q){
  ...
  const secrets={
    ...
    'midnight':()=>{const h=DATA.hourly||[];toast('深夜共留下 '+human(nightTokens(h))+' Token。',4200);},
    'receipt':()=>openShare('receipt'),'passport':()=>openShare('passport'),'flow':()=>scrollToSection('section-flow'),'orbit':()=>scrollToSection('section-flow'),'city':()=>scrollToSection('section-flow')
...
function finalizeSeason(rows,index,range,previous){
  const first=rows[0],last=rows[rows.length-1],models=modelTotals(rows);
  const total=sumBy(rows,'total'),calls=sumBy(rows,'calls'),cache=sumBy(rows,'cache'),dominant=topModelOf(models,true),peak=rows.reduce((best,row)=>!best||row.total>best.total?row:best,null),streak=longestActiveStreak(rows.map(row=>({period:row.day,total:row.total})));
...
function summarizeAlmanacSnapshot(rows,seasons,records,data=DATA){const models=modelTotals(rows);return {id:data.snapshot?.id||['legacy',data.generated,rows.length,sumBy(rows,'total')].join(':'),generated:String(data.generated||''),coverage:data.snapshot?.coverage||{first_day:rows[0]?.day||null,last_day:rows[rows.length-1]?.day||null},totals:{tokens:sumBy(rows,'total'),calls:sumBy(rows,'calls'),cache:sumBy(rows,'cache')},models,records:records.map(({id,label,unit,value,achievedDay,precision,detail,format})=>({id,label,unit,value,achievedDay,precision,detail,format})),seasons:seasons.map(({id,start,end,total,calls,activeDays,spanDays,dominantModel,dominantShare,peakDay,peakValue,longestStreak,character})=>({id,start,end,total,calls,activeDays,spanDays,dominantModel,dominantShare,peakDay,peakValue,longestStreak,character}))};}
...
function dominantModelFromSnapshot(snapshot){return topModelOf(snapshot?.models,true)?.[0]||null;}
...
let _almanac=null,seasonCursor=0,capsuleCursor=0;
...
function getBadgeData(){
  ...
  const days=DATA.day||[], total=sumBy(days,'total'), calls=sumBy(days,'calls'), cr=DATA.cache_read||0, cRatio=total?cr/total:0, models=DATA.models.length;
  ...
  // 每个用过模型一枚
  const modelItems=(DATA.models||[]).map((m,i)=>{const tot=sumWhere(days,d=>d.models[m]||0);return {e:'🤖',n:pretty(m)+'用户',d:'用过 '+pretty(m),tier:tierFor(i,Math.max(1,DATA.models.length)),ok:tot>0};});
...
function renderReplaySeries(){
  ...
  document.getElementById('replay-sub').textContent=s.length+' 轮'+(s.length>=200?'（最近 200 轮）':'')+' · 共 '+human(sumList(s))+' token · 横轴为轮次，不代表真实耗时';
...
function drawECG(index){
  ...
  const cur=Math.max(0,Math.min(n-1,Math.round(index))),cx=cur/Math.max(1,n-1)*W,cy=H-6-(s[cur]||0)/max*(H-12),total=sumList(s),cum=sumList(s.slice(0,cur+1));rp.i=cur;
...
(function(){
  document.addEventListener('dblclick',e=>{
    ...
    const cs=activeModelColors();for(let i=0;i<38;i++){const d=document.createElement('i');d.style.cssText='position:fixed;z-index:110;pointer-events:none;left:'+e.clientX+'px;top:'+e.clientY+'px;width:'+(3+i%4)+'px;height:'+(3+i%4)+'px;border-radius:50%;background:'+(cs[i%Math.max(1,cs.length)]||'#7aa2f7')+';transition:transform .85s cubic-bezier(.15,.7,.2,1),opacity .85s';document.body.appendChild(d);requestAnimationFrame(()=>{const a=i/38*Math.PI*2,r=40+(i%9)*9;d.style.transform='translate('+Math.cos(a)*r+'px,'+Math.sin(a)*r+'px) scale(.2)';d.style.opacity='0';});setTimeout(()=>d.remove(),900);}
  });
})();

// 光标彗星：单 Canvas + 固定粒子池，不持续创建 DOM
(function(){
  const canvas=document.getElementById('comet-canvas'),ctx=canvas.getContext('2d'),particles=Array.from({length:28},()=>({life:0}));let cursor=0,raf=0,last=0,allocated=false;
...
document.addEventListener('visibilitychange',()=>{if(document.hidden){if(rp.timer){clearInterval(rp.timer);rp.timer=null;document.getElementById('replay-play').textContent='▶ 播放';}}});
...
function reconcileLiveData(next){
  ...
  _almanac=null;_ach=null;previousModels=null;
