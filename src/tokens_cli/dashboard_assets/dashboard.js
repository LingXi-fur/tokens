...
const I18N_PATTERNS = Object.freeze([
  ...
  [/^原创日系 Furry 伙伴 · (狼|狐狸|猫|兔|龙) · 数据成长 (\d+)\/(\d+) · 当前外观仅在本地生成$/,(_,species,stage,total)=>`Original Japanese-inspired Furry companion · ${{'狼':'Wolf','狐狸':'Fox','猫':'Cat','兔':'Rabbit','龙':'Dragon'}[species]} · Data growth ${stage}/${total} · Generated locally only`],
  [/^成长 (\d+)\/(\d+)$/,(_,stage,total)=>`Growth ${stage}/${total}`],
...
function creatureDataPalette(metrics){const hue=(Math.floor(metrics.total||0)%210)+80,accent=(hue+55+metrics.models*9)%360;return {primary:hslCreatureColor(hue,58,55),secondary:hslCreatureColor((hue+22)%360,64,84),accent:hslCreatureColor(accent,75,64)};}
...
function creatureSpeciesParts(species,primary,secondary,accent){
  const parts={
    wolf:'<path d="M64 105 82 28 128 78M236 105 218 28 172 78" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="M76 83 84 48 112 82M224 83 216 48 188 82" fill="'+accent+'" opacity=".72"/>',
    fox:'<path d="M60 112 71 19 130 82M240 112 229 19 170 82" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="M72 84 78 44 116 82M228 84 222 44 184 82" fill="'+secondary+'"/><path d="M221 214q58 3 43 55-11 38-62 18 36-10 19-73Z" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="M247 254q8 26-28 30 23-15 10-34Z" fill="'+secondary+'"/>',
    cat:'<path d="M66 111 77 38 127 82M234 111 223 38 173 82" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="M80 82 84 57 112 84M220 82 216 57 188 84" fill="'+accent+'" opacity=".72"/><path d="M221 223q52 2 49 42-2 28-29 28 17-17-1-29-10-7-25-5" fill="none" stroke="'+primary+'" stroke-width="20" stroke-linecap="round"/>',
    rabbit:'<path d="M82 100Q58 30 88 14q28 13 35 72M218 100q24-70-6-86-28 13-35 72" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="M87 78Q74 37 88 27q14 15 20 56M213 78q13-41-1-51-14 15-20 56" fill="'+accent+'" opacity=".68"/><circle cx="238" cy="250" r="24" fill="'+secondary+'" stroke="#27344d" stroke-width="5"/>',
    dragon:'<path d="M78 92 63 42 111 72M222 92l15-50-48 30" fill="'+accent+'" stroke="#27344d" stroke-width="5"/><path d="M104 76 116 31 139 70M196 76l-12-45-23 39" fill="'+secondary+'" stroke="#27344d" stroke-width="5"/><path d="M225 216q55 20 35 72l-20-20-22 22q22-44-4-62Z" fill="'+primary+'" stroke="#27344d" stroke-width="5"/><path d="m254 256 18-5-10 17Z" fill="'+accent+'"/>'
  };return parts[species]||parts.wolf;
}
function renderCreature(){
  const svg=document.getElementById('creature'),metrics=creatureMetrics(),prefs=loadCreaturePrefs(),species=prefs.species==='auto'?defaultCreatureSpecies(metrics):prefs.species,dataPalette=creatureDataPalette(metrics),primary=prefs.primary||dataPalette.primary,secondary=prefs.secondary||dataPalette.secondary,accent=prefs.accent||dataPalette.accent,meta=CREATURE_SPECIES_META[species],stage=metrics.stage;
  ...
  const spots=Math.min(12,Math.ceil(metrics.projects/2)),ribbons=Math.min(5,metrics.models),eyeHeight=metrics.night>.42?12:9,p=[];
  p.push('<defs><filter id="fur-shadow"><feDropShadow dx="0" dy="8" stdDeviation="7" flood-opacity=".25"/></filter><linearGradient id="fur-body" x1="0" y1="0" x2="1" y2="1"><stop stop-color="'+secondary+'"/><stop offset=".48" stop-color="'+primary+'"/><stop offset="1" stop-color="'+accent+'"/></linearGradient><clipPath id="fur-face"><path d="M76 142Q76 72 150 65q74 7 74 77v50q-5 78-74 81-69-3-74-81Z"/></clipPath></defs>');
  p.push('<ellipse cx="150" cy="294" rx="76" ry="12" fill="rgba(0,0,0,.14)"/>',creatureSpeciesParts(species,primary,secondary,accent));
  p.push('<path d="M61 303q5-73 50-91h78q45 18 50 91Z" fill="'+primary+'" stroke="#27344d" stroke-width="5" filter="url(#fur-shadow)"/><path d="M91 303q8-62 59-73 51 11 59 73Z" fill="'+secondary+'" opacity=".92"/>');
  p.push('<path d="M76 142Q76 72 150 65q74 7 74 77v50q-5 78-74 81-69-3-74-81Z" fill="url(#fur-body)" stroke="#27344d" stroke-width="5" filter="url(#fur-shadow)"/>');
  p.push('<path d="M80 145q22-35 40-55l8 27 22-42 22 42 9-27q20 20 39 55-42-23-70-19-28-4-70 19Z" fill="'+primary+'" opacity=".9"/>');
  p.push('<path d="M102 182q16-31 48-24 32-7 48 24-7 62-48 66-41-4-48-66Z" fill="'+secondary+'"/>');
  p.push('<g class="creature-eye"><ellipse cx="119" cy="160" rx="10" ry="'+eyeHeight+'" fill="#172033"/><ellipse cx="181" cy="160" rx="10" ry="'+eyeHeight+'" fill="#172033"/><circle cx="116" cy="156" r="3" fill="#fff"/><circle cx="178" cy="156" r="3" fill="#fff"/></g><path d="m142 188 8-6 8 6-8 7Z" fill="'+accent+'" stroke="#27344d" stroke-width="2"/><path d="M150 195q-3 12-15 12m15-12q3 12 15 12" fill="none" stroke="#27344d" stroke-width="3" stroke-linecap="round"/>');
  p.push('<path d="M91 206q-18 3-27 14m31-4q-20 10-26 23m140-33q18 3 27 14m-31-4q20 10 26 23" fill="none" stroke="'+secondary+'" stroke-width="5" stroke-linecap="round"/>');
  for(let i=0;i<spots;i++){const x=98+(i*31)%105,y=103+((i*47)%72);p.push('<circle cx="'+x+'" cy="'+y+'" r="'+(2+i%3)+'" fill="'+accent+'" opacity="'+(.26+metrics.cache*.55).toFixed(2)+'" clip-path="url(#fur-face)"/>');}
  for(let i=0;i<ribbons;i++){const x=112+i*(76/Math.max(1,ribbons-1));p.push('<path d="M'+x+' 263v'+(20+i%2*8)+'" stroke="'+accent+'" stroke-width="4" stroke-linecap="round"/><circle cx="'+x+'" cy="'+(287+i%2*8)+'" r="5" fill="'+accent+'"/>');}
  if(metrics.cache>.35)p.push('<path d="M126 86q24-29 48 0" fill="none" stroke="'+accent+'" stroke-width="5" stroke-linecap="round"/><path d="m150 61 9 15-9 9-9-9Z" fill="'+accent+'" stroke="#fff" stroke-width="2"/>');
  if(stage>=4)p.push('<path d="M74 250 40 228l18 43-26 19 47 1" fill="'+accent+'" opacity=".72"/><path d="m226 250 34-22-18 43 26 19-47 1" fill="'+accent+'" opacity=".72"/>');
  svg.innerHTML=p.join('');document.getElementById('creature-name').textContent=(metrics.night>.42?'午夜':metrics.cache>.55?'晶核':metrics.models>=5?'虹彩':'星尘')+'·'+meta.title;document.getElementById('creature-desc').textContent='原创日系 Furry 伙伴 · '+meta.label+' · 数据成长 '+stage+'/5 · 当前外观仅在本地生成';document.getElementById('creature-info').innerHTML='<div><b>成长 '+stage+'/5</b>数据阶段</div><div><b>'+meta.label+'</b>伙伴物种</div><div><b>'+ribbons+'</b>模型饰带</div><div><b>'+spots+'</b>项目星斑</div><div><b>'+Math.round(metrics.cache*100)+'%</b>缓存晶核</div><div><b>'+Math.round(metrics.night*100)+'%</b>夜行倾向</div>';
}
...
function updateCreaturePrefs(){saveCreaturePrefs(currentCreaturePrefs());renderCreature();}
...
function initLiveDashboard(){
  if(!LIVE?.enabled){liveStatus('static','静态快照','静态离线快照');return;}
  let timer=null,failures=0,controller=null,current=DATA.snapshot?.id||null;
  const base=Math.max(1000,Number(LIVE.interval||5)*1000);
  const schedule=delay=>{clearTimeout(timer);if(!document.hidden)timer=setTimeout(check,delay);};
  async function check(){
    if(document.hidden)return;
    liveStatus('syncing','正在检查');controller?.abort();controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),Math.min(10000,base));
    try{
      const response=await fetch('/api/snapshot',{cache:'no-store',headers:current?{'If-None-Match':'"'+current+'"'}:{},signal:controller.signal});
      if(response.status===304){failures=0;liveStatus('live','本地实时');schedule(base);return;}
      if(!response.ok)throw new Error('status');
      const payload=await response.json();
      if(payload.error){failures++;liveStatus('error','同步错误',payload.error);schedule(Math.min(base*Math.max(2,failures),30000));return;}
      if(payload.snapshot&&payload.snapshot!==current){current=payload.snapshot;reconcileLiveData(decodeWire(payload.wire));liveStatus('updated','刚刚更新');setTimeout(()=>{if(!document.hidden)liveStatus('live','本地实时');},1800);}
      else liveStatus('live','本地实时');
      failures=0;schedule(base);
    }catch(error){if(error.name==='AbortError'&&document.hidden)return;failures++;liveStatus('offline','暂时断开');schedule(Math.min(base*Math.max(2,failures),30000));}
    finally{clearTimeout(timeout);}
  }
  document.addEventListener('visibilitychange',()=>{if(document.hidden){clearTimeout(timer);controller?.abort();}else check();});
  liveStatus('live','本地实时');schedule(base);
}
