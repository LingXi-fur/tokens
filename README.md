...
- 数据时刻：从当前模型筛选中提取范围末端静默日、最新 Token 里程碑、项目首次出现和每日主力模型接力；点击可直接进入对应日期的时光探针；
...
- **数据可信度实验室**：展示时间戳、项目、会话、Token 组成和回放覆盖率；按来源解释统计口径，标记全景 / 趋势 / 基础数据能力，并可复制不含 cwd、session 或逐轮明细的体检摘要；
