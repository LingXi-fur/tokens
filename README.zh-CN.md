...
# 生成标识脱敏的可分享版本
tokens dashboard --anonymize --open
```
...
- 模型筛选使用普通复选 chip，并提供可见的全选、清空和单步撤销；不依赖双击、右键或组合点击；
- 模型筛选会同步重算 Token、调用次数、缓存、Top、项目透镜、作息与导出内容；
- 点击柱子进入“时光探针”，对单日、单周或单月重算局部视图；探针内可直接启动 **Data Trail · 数据寻迹**，也可随时按 `I` 从当前全局范围开始；
- 数据寻迹是一条非模态、可退出的证据路径：先确认范围，再临时选择模型线索，随后在项目、会话与 Context 构成三个平行分支中选一个，最后跳转到项目透镜、会话回放或 Context Reuse River；临时模型不会修改全局筛选，除非明确点击“只看此模型”；
- 数据寻迹中的项目与会话只共享当前范围和临时模型，保持为分离聚合，不推断项目到会话的关联；会话回放仍展示报告内实际保留的完整会话序列，不按寻迹模型或探针周期二次裁剪；
- **Signal Lens · 信号透镜**：悬停或键盘聚焦模型、周期、项目与会话会临时预览兼容模块；点击或 Enter / Space 可固定信号，并从统一面板选择“只看模型”、进入时光探针、打开项目透镜或会话回放；透镜状态只存在于当前页面内存；
- **Exactness Key · 精确层**：按住 `Alt / Option` 临时显露精确 Token、分子 / 分母、调用记录与对比边界，也可用透镜中的可见按钮固定；总量与项目 Token 仍提供显式复制按钮；
- **Hold to Compare · 按住对比**：按住趋势区按钮约 240ms，或按住 `C`，临时叠加上一期轮廓；短按按钮固定 / 取消对比。只有固定状态写入视图 URL；
...
- 数据时刻：事件计算保留在趋势注释轨道与 Signal Lens 中，不再占用独立卡片；它提取范围末端静默 / 连续活跃、本报告范围累计里程碑、范围内首次观察项目和连续自然日的唯一主力模型变化；安静日、日期缺口和并列主力会打断接力，标记可用方向键与 Enter / Space 回看；
...
- **Token 流光图**：展示真实聚合的“项目 → 模型 → 会话”流向；光带宽度对应 Token 量；悬停或键盘聚焦可预览，点击固定到 Signal Lens，再从统一面板选择深入动作；当前视图可导出 SVG；
...
### Data Trail：把卡片串成一条证据路径

Data Trail 不增加新的数据口径，也不强迫用户按向导操作；项目透镜、复用之河和会话回放仍可直接打开。它只把当前页面状态组织成一条渐进式路径：

1. **确认范围**：显示当前日 / 周 / 月粒度、时光探针范围、模型覆盖、Token 和调用记录数；上一期是同粒度紧邻的上一条可用聚合记录，缺失时明确显示“无可比较上一期”。
2. **选择模型线索**：临时模型只影响这条寻迹，不会静默改动全局筛选；只有“只看此模型”会调用既有模型筛选。
3. **选择证据分支**：项目、会话和 Context 构成分别按当前范围与临时模型计算；字段不足时使用数据可信度能力说明解释原因，而不是把缺失当成零。
4. **深入既有模块**：项目进入项目透镜；会话打开报告范围内实际保留的完整回放；Context 进入复用之河并保留 Fresh Input、Output、Cache Read、Cache Write、Other 五分量。

语义边界：Token 增减只描述变化，不代表更好或更差；“调用记录”不等同于供应商计费请求；Cache Read 是缓存 Token 读取量，不等于已确认的货币节省；Other 是标准化分量与来源 `total` 对齐后的剩余量，非零时不会隐藏。不同 CLI 的 Token 定义仍以各自来源口径为准。

Data Trail 状态只存在于当前页面内存：步骤、临时模型、分支和项目 / 会话选择不会进入 URL、`localStorage`、成就快照或 Token 年鉴。关闭后会把焦点还给打开它的控件；它不是模态窗口，Tab 仍可离开轨道并直接使用其他模块。
...
| 打开 / 返回数据寻迹 | `I` |
| 寻迹步骤前后移动 | `←` / `→`（仅已到达步骤） |
| 浏览寻迹模型 / 证据分支 | `↑` / `↓` / `←` / `→`；`Home` / `End` |
| 选择线索 / 打开证据 | `Enter` / `Space` |
| 数据寻迹返回上一步 | `Backspace`（输入控件内不拦截） |
| 清除固定 Signal Lens；再退出时光探针 | `Esc` |
| 临时显露精确值与语义分母 | 按住 `Alt / Option` |
| 固定 / 取消精确层 | 点击 Signal Lens 中的“精确层” |
| 临时查看上一期轮廓 | 按住 `C`，或按住“按住对比”按钮约 240ms |
| 固定 / 取消上一期对比 | 短按“按住对比”按钮 |
| 切换日 / 周 / 月 | `1` / `2` / `3` |
| 打开命令面板 | `Cmd/Ctrl + K` |
| 切换自动 / 亮色 / 暗色主题 | `T` |
| 导出当前筛选 CSV | `E` |
...
| 打开快捷键帮助 | `?` |
| 切换数据侦探洞察 | `←` / `→` |
| Signal Lens 预览 / 固定 | 悬停或聚焦；点击 / `Enter` / `Space` |
| 浏览年鉴赛季 / 时间胶囊章节 | `←` / `→`；`Home` / `End` 跳到首尾 |
| 打开赛季 / 纪录详情 | `Enter` / `Space` |
| 模型筛选 | 点击 chip；使用可见的全选 / 清空 / 撤销 |
| 复制总 Token 精确值 | 点击复制按钮 |
...
# 自定义日期范围
tokens all --since 2026-07-01 --until 2026-07-31
...
tokens dashboard --anonymize --since 2026-07-01 --until 2026-07-31
...
所有日期默认按系统本地时区转换；可用 `--timezone Asia/Shanghai` 等 IANA 名称显式覆盖。
...
flowchart LR
    ...
    B --> C[平台用户缓存目录<br>mtime + size 文件缓存]
...
`tokens dashboard --anonymize` 会生成独立的 `out/dashboard-anonymized.html`：
...
本地 `file:` 页面复制视图链接时只复制文件名和 UI 状态，不再复制本机绝对路径；粒度、模型选择、焦点日期、固定对比和主题仍会出现在链接中。Data Trail 的打开状态、步骤、临时模型、证据分支及项目 / 会话选择均为页面内存态；Signal Lens 的预览 / 固定信号、Exactness Key 状态和按住对比状态也只存在于页面内存。它们不写入链接，不新增 `localStorage` key，也不把项目或会话实体 ID 交给 Token 年鉴或成就快照。Signal Lens 在流光图中只高亮真实的项目 → 模型与模型 → 会话关系，不推断项目 → 会话配对。
...
详见 [数据与隐私文档](docs/zh/data-and-privacy.html)。
...
├── docs/                         # 英文主站与 zh/ 中文入口
├── tests/                        # CLI、读取器、Dashboard 与文档测试
└── .github/workflows/            # CI、Pages 与发布自动化
```
...
# 从源码重新生成原始与假名化 Dashboard
./run dashboard
./run dashboard --anonymize
...
完整中文文档位于 [`docs/zh/`](docs/zh/index.html)：

- [快速开始](docs/zh/getting-started.html)
- [CLI 参考](docs/zh/cli.html)
- [Dashboard](docs/zh/dashboard.html)
- [数据与隐私](docs/zh/data-and-privacy.html)
- [架构](docs/zh/architecture.html)
- [FAQ](docs/zh/faq.html)
